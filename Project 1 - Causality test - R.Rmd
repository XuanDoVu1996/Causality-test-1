---
title: "Project 1 - Causality test"
author: "Danny Vu"
date: "2024-02-29"
output:
  html_document:
...
Generally, our data is clean and does not have many missing values, except for the "gender" variable with 8 "NA" values. Missing data might cause sampling bias and give us unrepresentative sample, which affects the quality of statistical inference. There is no universal rule for the acceptable proportion of missing data, but a missing rate of 5% or less is inconsequential and more than 10% will give bias in statistical analysis. As our missing rate is only 4.7%, we apply list-wise deletion to remove those 8 observations without affecting further analysis too much.  
...
## 1.3) Challenges when measuring the average causal effect

Causal effect of a binary treatment is defined as the difference between two potential outcomes (with and without the treatment). In addition, an average causal effect (ATE) is defined as the average difference between potential outcomes under different treatment. As we can’t observe the counterfactuals, the gold standard of classical experiment (randomized controlled trials) is usually preferred with the difference-in-difference estimators to estimate the average causal effect. The randomization guarantees the average outcome between two groups are solely attributed to the treatment, while DiD address the confounding bias due to time trends.  

In our case, each viewer group watches only a livestream session having one of two framing conditions. From our understanding, in the experiment, the choice frame is implemented before one lucky draw in a livestream session, while the rejection frame is implemented before the lucky draw of the other livestream session. The two livestream sessions happen in sequential order.  Given the above experiment design, the following problems would arise: 
